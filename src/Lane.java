...
public class Lane extends Thread implements PinsetterObserver {
    ...
    public void run() {
        ...
        while (!gameFinished) {    // we have a party on this lane,
                                    // so next bower can take a throw
            ...
            for (Bowler currentThrower : (Vector<Bowler>) bowlers) {
                ...
                if (frameNumber == 9) {
                    finalScores[bowlIndex][gameNumber] = cumulScores[bowlIndex][9];
                    Scoring.addScoreHistory(currentThrower, cumulScores, bowlIndex);
                }
                setter.reset();
                bowlIndex++;
        ...
        if (gameFinished) {  //game must be finished
            EndGamePrompt egp = new EndGamePrompt(((Bowler) party.getMembers().get(0)).getNickName() + "'s Party");
            int result = egp.getResult();
            egp.distroy();
    ...
    private void markScore(Bowler Cur, int frame, int ball, int score) {
        ...
        curScore[index - 1] = score;
        scores.put(Cur, curScore);
        Scoring.getScore(Cur, frame, scores, cumulScores, bowlIndex, ball);
        publish(lanePublish());
    }
...
>>>>>>> ed3e28fa8a97af1a3764c21cadd7fe5200cff65c
